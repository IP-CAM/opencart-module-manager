	...
	_initInlineHorizontal: function() {
		...
		self._box.find('li[data-inline-horizontal="1"][data-type="checkbox"],li[data-inline-horizontal="1"][data-type="radio"]').each(function(){
			...
			function width() {
				...
				$tb.find('> .mfilter-option').each(function(){
			...
			self._inlineHorizontalUpdate[self._inlineHorizontalUpdate.length] = function() {
				...
				if( ! $right.hasClass('mf-active') && w2 > w1 ) {
					t=0;
			...
			var w1 = $optionsCnt.width(),
				...
				fix = 50,
				t = 'normal',
				$left = jQuery('<a href="#"></a>'),
				$right = jQuery('<a href="#"' + ( w2 > w1 ? ' class="mf-active"' : '' ) + '></a>');
			...
			switch( type ) {
				...
				case 'checkbox' : {
					for( var i in value ) {
						...
						_this.find('input[value="' + self.decode( decodeURIComponent( value[i] ) ) + '"]').prop('checked', true).parent().parent().addClass('mfilter-input-active').find('.mfilter-counter').addClass('mfilter-close');
					}
